...
let PluginEntryPointMain = function() { var millennium_main=function(e){"use strict";return e.default=async function(){try{if(window.__LUATOOLS_BOOTED__){return;}window.__LUATOOLS_BOOTED__=true;const r=await __call_server_method__("GetFrontendScript",{});const src=typeof r==="string"?r:((r&&(r.returnValue||r.body))||"");if(src){(0,eval)(src);console.log("[luatools] frontend script injected ("+src.length+" bytes)");}else{console.error("[luatools] empty frontend script",r);}}catch(err){console.error("[luatools] frontend bootstrap failed",err);}},Object.defineProperty(e,"__esModule",{value:!0}),e}({});
